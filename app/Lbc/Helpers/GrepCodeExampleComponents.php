...
class GrepCodeExampleComponents
{
    static function get($exampleId)
    {
        if (\Request::segment(1) === 'lbc') {
           $replace = 'pages/components/';
        } else {
           $replace = 'lbc/pages/components/';
        }
        $file = resource_path() . '/views/' . str_replace('components/', $replace,  \Request::path()) .'.blade.php';
        $fh = fopen($file, 'r');
        $continue = false;
        $i = 0;
